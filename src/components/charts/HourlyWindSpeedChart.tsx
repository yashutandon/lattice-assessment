...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div className="bg-slate-800 border border-gray-600/50 rounded-lg px-3 py-2 text-xs shadow-xl">
      ...
      <p className="text-green-300 font-medium">{payload[0]?.value?.toFixed(1)} km/h</p>
    </div>
  );
}

export default function HourlyWindSpeedChart({ data }: { data: HourlyDataPoint[] }) {
  return (
    <ChartWrapper title="Wind Speed 10m (km/h)" minWidth={700}>
